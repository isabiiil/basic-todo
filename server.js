...
app.get('/get', async (req, res) => {
  ...
  try {
    ...
    const recentTasksQuery = pool.query(
      'SELECT * FROM tasks ORDER BY due DESC LIMIT 5'
    );
...
app.post('/post', async (req, res) => {
  ...
  if (!task || (task !== 'TABS' && task !== 'SPACES')) {
    return res.status(400).send('Invalid task specified.').end();
  }

  pool = pool || (await createPoolAndEnsureSchema());
  // [START cloud_sql_mysql_mysql_connection]
  try {
    const stmt = 'INSERT INTO todolist (task, status, priority, due) VALUES (?, ?, ?, ?)';
    // Pool.query automatically checks out, uses, and releases a connection
    // back into the pool, ensuring it is always returned successfully.
    await pool.query(stmt, [task, 'Pending', 3, timestamp]);
  } catch (err) {
  ...
  res.status(200).send(`Successfully added ${task} at ${timestamp}`).end();
});
